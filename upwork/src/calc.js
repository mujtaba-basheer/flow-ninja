window.addEventListener("load", () => {
    ...
    const groupEls = stickyTotalEl.querySelectorAll("div.calculator-cell:not(:last-child)");
    ...
    const updateBlockTotal = (inputEls, blockTotalEl, sideTotalEl) => {
        let blockTotal = 0;
    ...
    const updateExplore = (val, exploreBlockEl, exploreEl, i, j) => {
        let fVal = "0";
        ...
        if (val.length > 1) {
            ...
            const exploreEls = exploreBlockEl.querySelectorAll("a div.price-wrap div:not(:first-child)");
            let hide = true;
            ...
            if (hide)
                exploreBlockEl.style.display = "none";
        }
        exploreEl.setAttribute("href", href + val);
        const priceEl = document.getElementById(`gr-${numArrays[i]}-${numArrays[j]}-price`);
        if (priceEl)
            priceEl.textContent = fVal;
    };
    ...
    exploreBlocks.forEach((exploreBlock) => {
        ...
        linkEls.forEach((linkEl) => linkEl.setAttribute("data-href", linkEl.getAttribute("href")));
    });
    const calcBlocks = document.querySelectorAll("div.calculator-block:not(.bordered-block)");
    calcBlocks.forEach((calcBlock, i) => {
        const blockTotalEl = calcBlock.querySelector(`input#gr-${numArrays[i]}-total`);
        const sideTotalEl = document.querySelector(`div#gr-${numArrays[i]}-total-side`);
        const inputEls = calcBlock.querySelectorAll(`input:not(#gr-${numArrays[i]}-total)`);
        const exploreBlockEl = calcBlock.querySelector("div.explore-block");
        inputEls.forEach((inputEl, j) => {
            const exploreEl = document.getElementById(`gr-${numArrays[i]}-${numArrays[j]}-link`);
