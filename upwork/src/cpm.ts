type FieldName = "cost" | "cpm" | "impression";
type State = {
  active: Set<string>;
  disabled: null | FieldName;
  values: {
    cost: null | number;
    cpm: null | number;
...
window.addEventListener("load", (ev: any) => {
  const inputFieldIDs: FieldName[] = ["cost", "cpm", "impression"];
  const state: State = {
    active: new Set(),
    disabled: null,
    values: {
  ...
  const formEl: HTMLFormElement = document.querySelector(
    "#wf-form-Conversion-Form"
  );
  const resultEl: HTMLElement = document.querySelector(
    "div.calculator-results-wrapper"
  );
  const resetBtn = document.getElementById("reset-button");
  const conversionEl: HTMLElement = document.querySelector(
    "div.conversion-rate-section"
  );
  formEl.reset();

  const toggleText = (field: null | FieldName) => {
    const textEl = document.querySelector<HTMLElement>("div.calc-info");
    if (field) {
  ...
  const onInput = (val: string, field: FieldName) => {
    if (!val) {
      state.disabled = null;
