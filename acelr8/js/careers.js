const fetchJobs = async () => {
    try {
        const request = await fetch("https://api.ashbyhq.com/posting-api/job-board/acelr8");
        const response = await request.json();
        const { jobs } = response;
        const rootEl = document.querySelector("div.main-grid.z-bump");
        if (rootEl) {
            rootEl.querySelectorAll("a").forEach((el) => el.remove());
            for (const job of jobs) {
                const { id, title, location, jobUrl, applyUrl, publishedAt } = job;
                const jobCardEl = document.createElement("a");
                jobCardEl.id = id;
