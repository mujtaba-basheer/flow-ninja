...

  {
    ...
    showRoiBtn.on("click", function () {
      if (!formSubmitted) {
        ...
        if ($(window).width() <= 767) {
          setTimeout(() => $("#open-form").trigger("click"), 4000);
        }
      }
    });
    ...
    submitBtn.on("click", function () {
      ...
      ChiliPiper.submit("klara", "roi_calculator", {
        ...
        onSuccess: function () {
          ...
          $("#third-done-icon").addClass("show");
          setTimeout(
            () => $("#chilipiper-date-picker").css("display", "none"),
            7000
          );
          setTimeout(() => {
            let successId = "#successMessage1";
            if ($(window).width() <= 767) successId = "#successMessage2";
            $(successId).removeClass("hidden").css("z-index", "2");
          }, 8000);
        },
      });
    });
