...
const transform: (value: number) => number = (value: number) =>
  Math.round(
    ((Math.exp((power * value) / maxPackages) - 1) / (Math.exp(power) - 1)) *
      maxPackages
  );

const reverse: (value: number) => number = (value: number) =>
  (1 / power) *
  Math.log(((Math.exp(power) - 1) * value) / maxPackages + 1) *
  maxPackages;

const packageBasePrice: (packages: number) => number = (packages: number) => {
...
window.addEventListener("load", () => {
  ...
  const sliderValEl = document.getElementById("slider-value") as HTMLDivElement;
  const dispValEl = document.getElementById("real-value") as HTMLInputElement;
  const [starterCardEl, proCardEl, ,] =
  ...
  const getPricing: (packages: number) => number = (packages: number) => {
  ...
  const calc: () => void = () => {
    ...
    // Starter Card
    {
      const firstStepEl = starterCardEl.querySelector(
        ".first-step-starter"
      ) as HTMLDivElement;
      const secondStepEl = starterCardEl.querySelector(
        ".second-step-starter"
      ) as HTMLDivElement;
      const thirdStepEl = starterCardEl.querySelector(
        ".third-step-starter"
      ) as HTMLDivElement;
      ...
      if (pricing * packages >= minPro) {
        ...
        if (thirdStepEl) {
          ...
          if (priceEl)
            priceEl.textContent = `Upgraded to Pro above ${
              currencyEur ? "€" : "$"
            }${minPro} / month`;
        }
        ...
        if (pricing * packages < minStarter) {
          const priceEl =
            firstStepEl.querySelector<HTMLSpanElement>("span#starter-price");
          if (priceEl)
          ...
          const priceElpp = secondStepEl.querySelector<HTMLSpanElement>(
            "span#starter-price-second-step"
          );
          if (priceElpp)
            priceElpp.textContent = formatPrice(
              pricing,
              2,
              "en",
              currencyEur ? "EUR" : "USD"
          ...
          const priceElpm = secondStepEl.querySelector<HTMLSpanElement>(
            "span#second-step-starter-per-month"
          );
          if (priceElpm)
            priceElpm.textContent = formatPrice(
              pricing * packages,
              2,
              "en",
              currencyEur ? "EUR" : "USD"
    ...
    // Pro Card
    {
      const firstStepEl = proCardEl.querySelector(
        ".first-step-pro"
      ) as HTMLDivElement;
      const secondStepEl = proCardEl.querySelector(
        ".second-step-pro"
      ) as HTMLDivElement;
      const thirdStepEl = proCardEl.querySelector(
        ".third-step-pro"
      ) as HTMLDivElement;
      const linkEl = proCardEl.querySelector("a") as HTMLAnchorElement;
      ...
      if (packages >= maxPackages) {
        ...
        if (pricing * packages < minPro) {
          const priceEl =
            firstStepEl.querySelector<HTMLSpanElement>("span#pro-price");
          if (priceEl)
            priceEl.textContent = formatPrice(
              minPro,
              0,
          ...
          const priceElpp = secondStepEl.querySelector<HTMLSpanElement>(
            "span#pro-price-second-step"
          );
          const priceElpm = secondStepEl.querySelector<HTMLSpanElement>(
            "span#second-step-pro-per-month"
          );
          if (priceElpp)
            priceElpp.textContent = formatPrice(
              pricing,
    ...
    // Feedback
    if (feedbackEl)
      feedbackEl.textContent = formatPrice(
        pricing * feedbackMultiple,
        2,
        "en",
        currencyEur ? "EUR" : "USD"
      );
  ...
  const handleInputChange = (event: Event & { inputType: string }) => {
    if (event.type === "input" && event?.inputType) {
      const trackWidth = trackEl?.clientWidth;
      let packages = 0;
      try {
        packages = Math.min(
          parseInt(dispValEl.value.replace(" ", ""), 10),
          maxPackages
        );
        packages = packages || 0;
...

        {
          ...
          const fillEl = trackEl.querySelector<HTMLDivElement>(
            ".fs-rangeslider_fill"
          );
          if (fillEl) fillEl.style.width = `${fillWidth}px`;
          const handleEl = trackEl.querySelector<HTMLDivElement>(
