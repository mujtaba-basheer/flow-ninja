...
for (const formEl of formEls) {
    ...
    $(formEl)
        ...
        var element = $(formEl).find("#order-slider"), value = element.val(), min = 0, step;
        if (value < 200) {
            step = 100;
        }
        else if (value < 1999) {
    ...
    $(formEl).on("submit", function () {
        let actionValue = formEl.querySelector("#order-slider").value +
    ...
    setTimeout(function () {
        ...
        mainCountrySelect.forEach((item) => {
            ...
            function countryToggle() {
                ...
                let additionalCountriesList = formEl.querySelectorAll(".multiple-country");
