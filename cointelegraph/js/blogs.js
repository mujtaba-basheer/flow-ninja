window.addEventListener("load", async () => {
    try {
        ...
        let wishlist_items = await Wized.data.get("r.3.d.result_1.liked_wishlist_items");
        if (!wishlist_items)
            wishlist_items = [];
        let token = await Wized.data.get("c.token");
        // iterating through CMS items
        const cmsItems = document.querySelectorAll(".w-dyn-items .suggest-cms-item");
        cmsItems.forEach((cmsItem) => {
            const buttonEl = cmsItem.querySelector(".suggest-like-button-wrapper button");
            if (buttonEl) {
                const svgEl = buttonEl.querySelector("svg");
                const numLikesEl = cmsItem.querySelector(`.suggest-like-button-wrapper div[wized="numOfLikes"]`);
                const slug = buttonEl.id;
                if (svgEl && numLikesEl) {
                    if (wishlist_items.includes(slug))
                    ...
                    buttonEl.addEventListener("click", async () => {
                        try {
                            ...
                            const req = await fetch("https://xftf-jpdt-k3rz.n7c.xano.io/api:Mrj0XxDe/likes", {
                                method: "POST",
                                headers: {
