...
const LAMBDA_PROXY_URL = "https://0a50cfhnal.execute-api.us-east-1.amazonaws.com/sandbox";
...
const getGCFormValues = (gate, keys) => {
    return keys.map((k) => { var _a; return (_a = gate.form.getFieldById(k)) === null || _a === void 0 ? void 0 : _a.getValue(); });
};
const isAPIErrorException = (error) => {
    return error.request_id !== undefined;
};
const setView = () => {
    const defaultView = document.getElementById("default-block");
    const errorView = document.getElementById("error-block");
    const formView = document.querySelector(`div[gcdc-processed]`);
    switch (formStatus) {
        case 2: {
...
const preFormSubmission = async (event) => {
    ...
    const [first_name, last_name, work_email, trading_name] = getGCFormValues(gateInstance, ["first_name", "last_name", "email", "company"]);
    const nasApiData = {
        first_name,
        last_name,
        work_email,
