...
const { Engine, Render, Runner, Bodies, Body, Composite, MouseConstraint, Mouse, Events, Svg, Common, Vertices, Vector, } = Matter;
const setup = () => {
    ...
    let isFirst = true;
    let popAble = false;
    const imgUrlBase = "https://flow-ninja-assets.s3.amazonaws.com/bold/images";
    const data = {
        items: [
            {
                name: "Casio",
            ...
            {
                name: "Yazio",
                path: "M206.494 87.9279L58.1662 14.472C42.0269 6.47932 22.464 13.0835 14.4714 29.2228C6.47875 45.3622 13.0829 64.925 29.2223 72.9176L177.55 146.374C193.689 154.366 213.252 147.762 221.245 131.623C229.237 115.483 222.633 95.9206 206.494 87.9279Z",
                imgName: "yazio.png",
            },
            {
                name: "Aesop",
    ...
    const onMounted = () => {
        ...
        shuffleItems.forEach((item, i) => {
            ...
            matterRect.matterW / 4 +
                Math.round((Common.random() * matterRect.matterW) / 2), matterRect.matterH / 1.5, 
            // @ts-ignore
        ...
        // WALL ADD x,y,w,h
        const wallTop = Bodies.rectangle(matterRect.matterW / 2, 0 - 490, matterRect.matterW, 1000, { isStatic: true, label: "roof", render: { fillStyle: "transparent" } });
        const wallBottom = Bodies.rectangle(matterRect.matterW / 2, matterRect.matterH + 300, matterRect.matterW, 1000, {
            isStatic: true,
            label: "ground",
            render: { fillStyle: "transparent" },
        });
        const wallLeft = Bodies.rectangle(0 - 390, matterRect.matterH / 2, 800, matterRect.matterH, {
            isStatic: true,
            label: "wallLeft",
            render: { fillStyle: "transparent" },
        });
        const wallRight = Bodies.rectangle(matterRect.matterW + 390, matterRect.matterH / 2, 800, matterRect.matterH, {
            isStatic: true,
            label: "wallRight",
            render: { fillStyle: "transparent" },
        });
        const wallMinime = Bodies.rectangle((matterRect.matterW / 15) * 5.8, matterRect.matterH - 210, 380, 180, {
            isStatic: true,
            label: "wallMinime",
            render: { fillStyle: "transparent" },
        });
        ...
        const shakeScene = (engine, isStart) => {
        ...
        const resizeCanvas = () => {
            ...
            for (let i = 0; i < bodies.length; i++) {
                ...
                if (!body.isStatic) {
                    ...
                    Body.applyForce(body, body.position, {
                        x: body.position.x >= 500 && render.canvas.width > matterRect.matterW
                            ? -4
                            : 0,
        ...
        Events.on(mouseConstraint, "enddrag", function (event) {
            ...
            const xGap = Math.abs(event.mouse.mousedownPosition.x - event.mouse.mouseupPosition.x);
            const yGap = Math.abs(event.mouse.mousedownPosition.y - event.mouse.mouseupPosition.y);
