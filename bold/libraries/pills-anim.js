...
const setup = () => {
  ...
  const imgUrlBase = "../images/logos";
  const data = {
    items: [
      {
        name: "Casio",
        path: "m0,0h297v101h-297z",
  ...
  const method = {
    ...
    getMatterRect2: (w) => {
      ...
      if (cw <= 768) {
        ...
        matterW = 3200;
        matterH = (matterW * 500) / 1600;
      } else {
        matterW = 3800;
        matterH = (matterW * 800) / 1600;
      }
      // matterH += bottomWallGap; // bottom wall gap
      console.log({
        matterW: matterW,
        matterH: matterH,
  ...
  const onMounted = () => {
    ...
    if (canvasWrap) {
      render = Render.create({
        ...
        options: {
          ...
          height: window.innerHeight || matterRect.matterH,
    ...
    const resizeCanvas = () => {
      ...
      for (let i = 0; i < bodies.length; i++) {
        ...
        if (!body.isStatic) {
          ...
          Body.applyForce(body, body.position, {
            x:
              body.position.x >= 500 && render.canvas.width > matterRect.matterW
                ? -4
                : 0,
    ...
    window.addEventListener("click", () => shakeScene(engine));
    window.addEventListener("contextmenu", () => shakeScene(engine, true));
    ...
    Events.on(mouseConstraint, "mousemove", function (event) {
      if (dragBody !== null) {
        ...
        // ### object hit test!!..
        if (
          mousePosition.y <= bottomWallGap ||
          mousePosition.y >= render.canvas.height - bottomWallGap ||
          mousePosition.x <= 20 ||
