...
const setup = () => {
  ...
  const numSteps = 20.0;
  let boxElement: HTMLDivElement | null;
  let prevRatio = 0.0;
  ...
  const onMounted = () => {
    ...
    shuffleItems.forEach((item: ItemT, i) => {
    ...
    // Observer
    boxElement = document.querySelector<HTMLDivElement>("#logo-section");
    ...
    const handleIntersect: IntersectionObserverCallback = (
      ...
      entries.forEach((entry) => {
        if (entry.intersectionRatio > prevRatio) {
          ...
          (entry.target as HTMLDivElement).style.opacity =
