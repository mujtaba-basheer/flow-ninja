    ...
    rootEl.classList.add("wf-section");
    {
        ...
        containerEl.classList.add("grid-job-card");
        {
            ...
            jobPostEl.id = "w-node-_8b4b6cdc-a290-6ff3-b2a6-7a45891e54ff-1e00ef2b";
            {
                ...
                {
                    timePostedEl.textContent = `Posted ${job.days_since_posted} day${job.days_since_posted && job.days_since_posted > 1 ? "s" : ""} ago`;
                }
...
window.addEventListener("load", async () => {
    try {
        const jobsWrapperEl = document.querySelector(".gray-sections-wrapper");
        if (jobsWrapperEl) {
            jobsWrapperEl
                .querySelectorAll("div.section.profile-gray.wf-section")
                .forEach((x) => x.remove());
            const req = await fetch("https://z5ak1hwsf9.execute-api.us-east-1.amazonaws.com/prod/workable-jobs", {
                method: "GET",
            });
            if (req.status !== 200)
