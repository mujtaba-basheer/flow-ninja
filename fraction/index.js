...
const calc = () => {
    ...
    results.cash_pocket.regular_mortgage = -PV(constants.DISCOUNT_RATE / 12, term * 12, payment, mortgages[60].balance);
    results.cash_pocket.he_investment = -PV(constants.DISCOUNT_RATE, term, 0, (original_value * Math.pow(1 + appreciation, term) -
        original_value * (1 - constants.HE_INV_DISCOUNT)) *
        ((loan_amt / original_value) * constants.HE_INV_MULTIPLE) +
        loan_amt);
    results.cash_pocket.he_agreement = -PV(constants.DISCOUNT_RATE, term, 0, original_value *
        Math.pow(1 + appreciation, term) *
        ((loan_amt / original_value) * constants.HE_AGR_MULTIPLE) +
...
const onParamChange = () => {
    calc();
    if (results) {
        const competitor = document.querySelector("#competition").value;
        for (const param of Object.keys(results)) {
            ...
            if (competitionValMap[competitor]) {
                ...
                document.getElementById(`competition-${paramIdMap[param]}`).textContent = compVal;
            }
        }
    }
};
const onCompetitorChange = (competitor) => {
    for (const param of Object.keys(results)) {
        const compVal = results[param][competitionValMap[competitor]];
...
window.addEventListener("load", () => {
    ...
    Object.keys(idMaps.params).forEach((key) => {
        $(`#${idMaps.params[key]}`).on("change", onParamChange);
    });
