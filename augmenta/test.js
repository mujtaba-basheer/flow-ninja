  ...
  $(pgrDropdown).on("change", function () {
    ...
    if (isYes) {
      ...
      $(cropEl.querySelector("#regulator-block"))
        .addClass("hidden")
        .removeAttr("required");
      $(cropEl.querySelector("#Number-Of-In-Season-Applications-PGR")).addClass(
        "hidden"
      );
...
const calc = async () => {
  ...
  for (const year of years) {
    ...
    // mailData[`${year}_nVraFees`] = results[year]["nVraFees"];
    // mailData[`${year}_pgrVraFees`] = results[year]["pgrVraFees"];
    // mailData[`${year}_haVraFees`] = results[year]["haVraFees"];
